  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <form
        ...
        className="w-full max-w-2xl bg-[#1a1625] p-8 rounded-3xl border border-[#322654] relative animate-in fade-in zoom-in duration-200"
      >
        <button
          type="button"
          onClick={onClose}
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              <option value="Cultura de Identidade">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div
              className={`border-2 border-dashed rounded-2xl p-8 flex flex-col items-center cursor-pointer ${arquivo ? "border-[#8B5CF6]" : "border-[#322654]"}`}
            >
              <input
                type="file"
                className="hidden"
                id="file-upload"
                onChange={(event) =>
                  event.target.files && setArquivo(event.target.files[0])
                }
              />
              <label
                htmlFor="file-upload"
              ...
              >
                <FontAwesomeIcon
                  icon={arquivo ? faFileLines : faArrowUpFromBracket}
                  className={`text-3xl mb-2 ${arquivo ? "text-[#FF8300]" : "text-[#8B5CF6]"}`}
                />
        ...
        <div className="mt-10 flex gap-4">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 border border-[#322654] text-white py-4 rounded-xl font-bold"
          >
            Cancelar
          </button>
          <button
            type="submit"
            className="flex-1 bg-[#8B5CF6] text-white py-4 rounded-xl font-bold hover:bg-[#7c4dff]"
          >
            Enviar
          </button>
        </div>
